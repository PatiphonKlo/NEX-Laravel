<button type="button" id="share-quotation-button-<?php echo e($key); ?>" onclick="shareQuotationButton<?php echo e($key); ?>()" class="uppercase w-full inline-flex items-center px-3 py-2 text-xs font-medium text-center text-white bg-primary rounded-md hover:bg-green-800">
    ...
    <div class="fixed inset-0 z-10 w-screen">
        <div id="overlay-share-<?php echo e($key); ?>" class="flex min-h-full justify-center p-4 text-center items-center sm:p-0">
            <div class="p-5 relative transform rounded-lg bg-white text-left shadow-xl transition-all sm:max-w-3xl lg:max-w-6xl w-200">
                ...
                <!-- Modal body -->
                <div class="p-3 pr-8 overflow-y-auto h-[50vh]" action="<?php echo e(config('app.env') == 'production' ? secure_url('admin/share-password/'. $quotation['quotation_id']) : url('admin/share-pasword/' . $quotation['quotation_id'])); ?>" method="POST" enctype="multipart/form-data">
...
<?php endif; ?>
                    <div class="relative flex justify-end items-center mb-4">
                        <a data-copy-target='copiedLink<?php echo e($key); ?>' href="<?php echo e(url('pdf-access/quotation/'.$quotation['quotation_id'])); ?>" target="_blank" rel="noreferrer noopener" class="text-blue-600 w-full bg-gray-50 border border-gray-300 text-sm rounded-lg focus:ring-primary focus:border-primary p-2.5 pr-14 truncate">
...
<?php endif; ?>
                    <div class="relative flex justify-end items-center">
                        <input disabled value="<?php echo e($quotation['quotation_password'] ?? ''); ?>" type="text" data-copy-target="copyPassword<?php echo e($key); ?>" class="w-full bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-primary focus:border-primary p-2.5 pr-10 truncate">
...
<?php /**PATH C:\my-project\miscibles-platform\src\resources\views/pages/admin/quotation/modal-share.blade.php ENDPATH**/ ?>