@extends('layouts/admin',['title' => 'group Data CMS'])
...
<div class="p-2">
    <div class="block justify-between sm:flex">
        ...
        <div class="flex flex-row">
            ...
            @include('pages/admin/cost-estimation/modal-add')
        </div>
    </div>
</div>
...
<table id="group-table" class="min-w-full shadow-md">
    ...
    <tbody id="group-table-body" class="bg-white divide-y divide-gray-200">
        @if(isset($productGroupData))
        @foreach ($productGroupData as $key => $product_group)
        <tr class="hover:bg-gray-100 ">

            @if (isset($product_group['product_group']) && $product_group['product_group'] !== '' &&
            ...
            <td class="p-4 text-sm font-medium text-gray-900 whitespace-nowrap">
                ...
                <p class="text-xxs font-normal">{{ $product_group['product_group'] }}</p>
            </td>
            @else
            <td class="p-4 text-xs font-normal whitespace-nowrap">Data Not Found</td>
            @endif

            @if (isset($product_group['created_by']) && $product_group['created_by'] !== '' && isset($product_group['created_at']) &&
            $product_group['created_at'] !== '')
            <td class="p-4 text-sm font-medium text-gray-900 whitespace-nowrap">
                {{ $product_group['created_by'] }} <br>
                <p class="text-xxs font-normal">{{ $product_group['created_at'] }}</p>
            </td>
            @else
            <td class="p-4 text-xs font-normal whitespace-nowrap">Data Not Found</td>
            @endif

            @if (isset($product_group['edited_by']) && $product_group['edited_by'] !== '' && isset($product_group['edited_at']) &&
            $product_group['edited_at'] !== '')
            <td class="p-4 text-sm font-medium text-gray-900 whitespace-nowrap">
                {{ $product_group['edited_by'] }} <br>
                <p class="text-xxs font-normal">{{ $product_group['edited_at'] }}</p>
            </td>
            @else
            <td class="p-4 text-xs font-normal whitespace-nowrap">Data Not Found</td>
            @endif
            ...
            <td class="p-4 flex space-x-2">
                <div class="w-36">
                    @include('pages/admin/cost-estimation/modal-update')
                </div>
                <div class="w-24">
                    @include('pages/admin/cost-estimation/modal-delete')
                </div>
            </td>
        </tr>
        @endforeach
