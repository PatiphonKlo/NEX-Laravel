@extends('layouts/admin', ['title' => 'Quotation CMS'])
    ...
    <div class="p-2">
        <div class="block justify-between sm:flex">
            ...
            <div class="flex">
                ...
                @include('pages/admin/quotation/modal-add')
            </div>
        </div>
    </div>
                    ...
                    <tr class="count hover:bg-gray-100 ">
                        ...
                        @if (
                            ...
                            <td class="p-4 text-sm font-medium text-gray-900 whitespace-nowrap">
                                @if (isset($quotation['product_data']['product_name']) && $quotation['product_data']['product_name'] !== '')
                                    {{ $quotation['product_data']['product_model'] }} <br>
                                @else
                                    <p class="text-xs font-normal whitespace-nowrap">product not found</p>
                                @endif
                                <p class="text-xxs font-normal">{{ $quotation['quotation_code'] }}</p>
                            </td>
                        @else
                            <td class="p-4 text-xs font-normal whitespace-nowrap">Data Not Found</td>
                        @endif

                        @if (isset($quotation['client_data']['client_contact_name']) &&
                                $quotation['client_data']['client_contact_name'] !== '' &&
                                isset($quotation['created_at']) &&
                                $quotation['created_at'] !== '')
                            <td class="p-4 text-sm font-medium text-gray-900 whitespace-nowrap">
                                {{ $quotation['client_data']['client_contact_name'] }} <br>
                                <p class="text-xxs font-normal">{{ $quotation['created_at'] }}</p>
                            </td>
                        @else
                            <td class="p-4 text-xs font-normal whitespace-nowrap">Data Not Found</td>
                        @endif

                        @if (isset($quotation['edited_by']) &&
                                $quotation['edited_by'] !== '' &&
                                isset($quotation['edited_at']) &&
                                $quotation['edited_at'] !== '')
                            <td class="p-4 text-sm font-medium text-gray-900 whitespace-nowrap">
                                {{ $quotation['edited_by'] }} <br>
                                <p class="text-xxs font-normal">{{ $quotation['edited_at'] }}</p>
                            </td>
                        @else
                            <td class="p-4 text-xs font-normal whitespace-nowrap">Data Not Found</td>
                        @endif
                        ...
                        <td class="p-4 flex space-x-2">
                            <div class="w-24">
                                <a href="{{ url('admin/quotation-pdf/' . $quotation['quotation_id']) }}" target="_blank"
                            ...
                            <div class="w-24">
                                @include('pages/admin/quotation/modal-send')
                            </div>
                            <div class="w-24">
                                @include('pages/admin/quotation/modal-share')
                            </div>
                            <div class="w-24">
                                @include('pages/admin/quotation/modal-update')
                            </div>
                            <div class="w-24">
                                @include('pages/admin/quotation/modal-delete')
                            </div>
                        </td>
                    </tr>
                @endforeach
