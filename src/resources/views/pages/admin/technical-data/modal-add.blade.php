    ...
    <div class="fixed inset-0 z-10 w-screen">
        <div id="overlay" class="flex min-h-full justify-center p-4 text-center items-center sm:p-0">
            <div class="p-5 relative transform rounded-lg bg-white text-left shadow-xl transition-all w-full sm:max-w-xl md:max-w-2xl lg:max-w-4xl">
                ...
                <!-- Modal body -->
                <form class="p-3 sm:p-8 overflow-y-auto h-[40vh]" action="{{ config('app.env') == 'production' ? secure_url('admin/add-technical') : url('admin/add-technical')}}" method="POST" enctype="multipart/form-data">
                    @csrf
                    <div class="grid gap-4 mb-4 sm:grid-cols-2">
                        <div>
                            <label for="technical_component" class="block mb-2 text-sm font-medium text-gray-900">Technical Component</label>
                            <input type="text" name="technical_component" value="{{ old('technical_component') }}" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-primary focus:border-primary block w-full p-2.5" placeholder="Type technical component">
                            @error('technical_component')
                        ...
                        <div>
                            ...
                            <input type="number" name="energy_consumption" value="{{ old('energy_consumption') }}" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-primary focus:border-primary block w-full p-2.5" placeholder="Hr/Day">
                    ...
                    <div class="w-full">
                        ...
                        <input type="text" name="specification" value="{{ old('specification') }}" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-primary focus:border-primary block w-full p-2.5" placeholder="Type specification">
                        @error('specification')
                        <p class="mt-2 text-sm text-red-600 whitespace-normal">{{$message}}</p>
                        @enderror
                    </div>
...
@if(!($errors->has('technical_component') || $errors->has('energy_consumption') || $errors->has('specification')))
<script>
