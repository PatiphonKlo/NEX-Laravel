<button type="button" id="delete-technical-button-{{ $technical['technical_data_id'] }}"
    onclick="deleteTechnicalButton{{ $technical['technical_data_id'] }}()"
...
<div id="popup-modal-delete-technical-{{ $technical['technical_data_id'] }}" class="hidden relative z-30"
    aria-labelledby="modal-title" role="dialog" aria-modal="true">
    <div class="fixed inset-0 bg-gray-900 bg-opacity-50 transition-opacity"></div>
    <div class="fixed inset-0 z-10 w-screen overflow-y-auto">
        <div id="overlay-delete-technical-{{ $technical['technical_data_id'] }}"
            ...
            <div
                ...
                <button type="button" id="delete-technical-close-button-1-{{ $technical['technical_data_id'] }}"
                ...
                <div class="p-6 text-center">
                    ...
                    <h3 class="mb-5 text-lg font-normal text-gray-500">Are you sure you want to delete
                        ...
                        spec: {{ $technical['specification'] }}</h3>
                    <form action="{{ url('admin/delete-technical/' . $technical['technical_data_id']) }}" method="POST"
                        class="inline">
                        @csrf
                        @method('DELETE')
                        <button type="submit"
                    ...
                    <button type="button" id="delete-technical-close-button-2-{{ $technical['technical_data_id'] }}"
...
<script>
    function deleteTechnicalButton{{ $technical['technical_data_id'] }}() {
        let delete_technical_button = document.getElementById(
            'delete-technical-button-{{ $technical['technical_data_id'] }}')
        let delete_technical_modal = document.getElementById(
            'popup-modal-delete-technical-{{ $technical['technical_data_id'] }}')
        let delete_technical_close_button1 = document.getElementById(
            'delete-technical-close-button-1-{{ $technical['technical_data_id'] }}')
        let delete_technical_close_button2 = document.getElementById(
            'delete-technical-close-button-2-{{ $technical['technical_data_id'] }}')
        let overlay = document.getElementById('overlay-delete-technical-{{ $technical['technical_data_id'] }}');
