    ...
    <table id="quotaionTable">
        <thead>
            ...
            <tr>
                ...
                <th class="bg-grey" colspan="9" style="font-size: 18px; font-weight: bold;">
                    QUOTATION
                </th>
            </tr>

            <tr class="bg-grey">
                <th colspan="5">เอกสารใบเสนอราคา ประจำปี 2023</th>
                <th colspan="4">ISO 9001 : 2015</th>
            </tr>
            <tr class="bg-grey">
                <th colspan="1">Date:</th>
                <th colspan="4">{{ $quotation['created_at'] }}</th>
                <th colspan="1">Code:</th>
                <th colspan="3">{{ $quotation['quotation_code'] }}</th>
            </tr>
            <tr>
                <th colspan="9" class="bg-grey bold">Miscible Technology Co.,Ltd.</th>
            </tr>
            <tr>
        ...
        <tbody>
            ...
            <tr>
                ...
                <td colspan="6" class="dashed-right left">
                    ...
                    {{ $quotation['product_data']['product_model'] ?? '' }}
                </td>
                <td colspan="1" class="dashed-right">{{ $quotation['product_amount'] ?? 1 }}</td>
                <td colspan="2" class="dashed-right">
                    {{ number_format($quotation['product_price'] ?? 0) }}</td>
                <td colspan="2" class="dashed-right">
                    {{ number_format(($quotation['product_price'] ?? 0)*($quotation['product_amount'] ?? 1)) }}</td>
            </tr>
            @endif
            ...
            <tr>
                ...
                <td colspan="2" class="dashed-right">
                    {{ number_format(($quotation['product_data']['product_logistic_cost'] ??
                    0)*($quotation['product_amount'] ?? 1)) }}
                </td>
            </tr>
            @endif

            @for ($i = count($chunk); $i < 12; $i++) <tr>
                <td colspan="1" class="dashed-right"></td>
                ...
                <tr class="dashed">
                    <td colspan="12" class="dashed-right"> **หมายเหตุ** (บริษัท มิสซิเบิล เทคโนโลยี จำกัด
                        มีระยะเวลารับประกันสินค้า {{ $quotation['product_warranty'] ?? '' }} ปี )</td>
                </tr>
        </tbody>
        <tfoot>
            <tr>
            ...
            <tr>
                ...
                <th colspan="2" class="bg-dark-grey">
                    {{ number_format(((($quotation['product_price'] ?? 0) +
                    ($quotation['product_data']['product_logistic_cost'] ?? 0)) * ((100 - ($quotation['product_discount'] ?? 0))/100) *($quotation['product_amount'] ?? 1)), 2, '.', ',') }}
                </th>
            </tr>
            <tr>
                ...
                <th colspan="2" class="bg-dark-grey">
                    {{ number_format(((($quotation['product_price'] ?? 0) +
                    ($quotation['product_data']['product_logistic_cost'] ?? 0)) * ((100 - ($quotation['product_discount'] ?? 0))/100) * ($quotation['product_amount'] ?? 1) *
                    0.07), 2, '.', ',') }}
                </th>
            </tr>
            <tr>
                @php
                $totalPrice =
                ((($quotation['product_price'] ?? 0) + ($quotation['product_data']['product_logistic_cost'] ?? 0))* ((100 - ($quotation['product_discount'] ?? 0))/100) * ($quotation['product_amount'] ?? 1)) 
                +((($quotation['product_price'] ?? 0) + ($quotation['product_data']['product_logistic_cost'] ?? 0))* ((100 - ($quotation['product_discount'] ?? 0))/100) * ($quotation['product_amount'] ?? 1) * 0.07);
                @endphp

                <th colspan="2" class="bg-dark-grey">
                    {{ number_format(($totalPrice * (($quotation['product_down_payment'] ?? 0) / 100)), 2, '.', ',') }}
                </th>
                <th colspan="2" class="bg-dark-grey">
                    {{ number_format(($totalPrice * (($quotation['product_final_check'] ?? 0) / 100)), 2, '.', ',') }}
                </th>
                <th colspan="2" class="bg-dark-grey">
                    {{ number_format(($totalPrice * (($quotation['product_after_install'] ?? 0) / 100)), 2, '.', ',') }}
                </th>
            ...
            <tr>
                <th colspan="4" rowspan="4">
                    <img src="{{ public_path('images/image003.png') }}" width="45" height="30" style="margin: 7px;">
                    <img src="{{ public_path('images/image004.png') }}" width="45" height="30" style="margin: 7px;">
                    <img src="{{ public_path('images/image005.png') }}" width="45" height="30" style="margin: 7px;">
                    <img src="{{ public_path('images/image006.png') }}" width="45" height="25" style="margin: 7px;">
                    <img src="{{ public_path('images/image007.png') }}" width="45" height="30" style="margin: 7px;">
                    <img src="{{ public_path('images/image008.png') }}" width="45" height="25" style="margin: 7px;">
                </th>
                ...
                <th colspan="4" height="80">
                    <div class="flex-container">
                        <img src="{{ $quotation['signature'] }}" alt="" id="signature">
                    </div>
                </th>
            </tr>
            <tr>
